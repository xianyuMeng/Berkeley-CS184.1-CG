...
void display() 
{
	//clear color buffers, which are rgba, the default value is 0.
  glClearColor(0, 0, 1, 0);
  //clears buffers to preset values.
  //what's GL_ACCUM_BUFFER_BIT and GL_STENCIL_BUFFER_BIT ? 
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  // I'm including the basic matrix setup for model view to 
  // give some sense of how this works.  
  //The model view matrix combines both model an view matrices to
  //position objects such that the camera is the origin, lokking down -z dim. 
  glMatrixMode(GL_MODELVIEW);
  //Apply subsequence matrix oprations to the model view matrix satck
  mat4 mv; 
  ...
  if (numused) {
	...
	/*for (size_t i = 1; i <= numused ; ++i){
		const size_t array_size = 4;
		GLfloat input[array_size];
		GLfloat output[array_size];
		GLfloat inputcolor[array_size];
		for (size_t j = 0; j != array_size; ++j){
			input[j] = lightposn[(i - 1) * 4 + j];
			inputcolor[j] = lightcolor[(i - 1) * 4 + j];
		}
		transformvec(input, output);
		glUniform4fv(lightpos, 1, output);
		glUniform4fv(lightcol, 1, inputcolor);
  ...
  for (int i = 0 ; i < numobjects ; i++) {
	...
	glm::mat4 Trans = transf * obj->transform;
